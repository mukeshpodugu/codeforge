...
export class AiService {
  ...
  public static async evaluateCode({ code, problemTitle, problemDifficulty, language }: CodeEvaluationInput) {
    ...
    if (bugs.length > 0) qualityScore -= 10 * bugs.length;
  ...
  public static async analyzeResume({ fileName, skillsText, experienceText }: ResumeAnalysisInput) {
  ...
  public static async chatInterview(history: { role: 'user' | 'assistant'; content: string }[], currentMessage: string, type: 'Technical' | 'DSA' | 'HR') {
  ...
  public static async generateRoadmap(skillLevel: string, goals: string, weakTopics: string[]) {
    const steps: { title: string; desc: string; duration: string; resources: string[] }[] = [];
